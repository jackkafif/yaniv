...
class Play:
    ...
    def playModelMove(self, state: GameState) -> tuple[bool, bool]:
        ...
        action = (m1, m2, m3)
        possible = (p1, p2, p3)
        next_state, reward, done, win = self.model.play_step(
            state, action, False)
    ...
    def play(self):
        ...
        while input("Would you like to play Yaniv? Y/N: ") == "Y":
            ...
            while done != True:
                ...
                state, done, win = sim_step(
                    self.model, state, state.player_1_hand, state.player_2_hand)
                if done:
                ...
                valid_moves_english = [[our_hand[j]
                                        for j in i[1]] for i in vmvs]
                while move not in valid_moves:
                    print(
                        f"That move is invalid, please try again {valid_moves_english}")
                    move = input(
                        "Input the cards you want to play in their indices 0 1 2 ... ")
                    move = [int(i) for i in move.split(" ")]
                draw = input(
                    "-1 to draw from deck, 0 to draw first card, 1 to draw second card from pile ")
