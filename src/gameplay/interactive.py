...
def play(model : CombinedModel):
    ...
    while input("Would you like to play Yaniv? Y/N: ") == "Y":
        ...
        print("Dealing Cards...")
        while done != True:
            print(f"Your hand is: {state.hand_to_cards(state.player_1_hand)}")
            print(f"The card(s) on the top is/are {[state.card_to_name(card) for card in state.top_cards]} ")
            yaniv = input("Enter Y if you'd like to call Yaniv, anything else otherwise: ")
            ...
            draw = input("Input -1 to draw from deck, 0 to draw first index of discard, 1 for second: ")
            while True:
                try:
