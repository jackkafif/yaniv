...
def test_state():
    ...
    print(state.hand_to_cards(state.player_1_hand),
          state.get_hand_value(state.player_1_hand))

    valid_moves = state.valid_moves(state.player_1_hand)
    for idx in valid_moves:
        move = list(POSSIBLE_MOVES[idx])
        print(state.move_to_cards(state.player_1_hand, move),
              state.move_value(state.player_1_hand, move))
    ...
    valid_moves = state.valid_moves(hand1)
    print(valid_moves)
    print(state.get_moves_values(hand1, valid_moves))
