...
class MultipleLayers(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.linear2 = nn.Linear(input_size * 2, input_size * 4)
        self.bn2 = nn.LayerNorm(input_size * 4)
        self.linear3 = nn.Linear(input_size * 4 + input_size, output_size)
        self.relu = nn.ReLU()
    ...
    def forward(self, xo):
        ...
        x = self.bn2(x)
        x = self.relu(x)
        x = self.linear3(torch.cat([x, xo], dim=-1))
        return x
...
class SimpleNN(nn.Module):
    def __init__(self, input_size, output_size):
        super().__init__()
        self.layer = nn.Linear(input_size, output_size ** 2)
        self.relu = nn.ReLU()
        self.out = nn.Linear(output_size ** 2, output_size)
        self.path = "linear"
