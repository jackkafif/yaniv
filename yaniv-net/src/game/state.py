...
def playOpponentTurn(self) -> tuple[bool, bool]:
    ...
    our_hand_value = self.get_hand_value(self.player_2_hand)
    opponent_hand_value = self.get_hand_value(self.player_1_hand)
    

    if self.can_yaniv(self.player_2_hand) and our_hand_value < min(7, opponent_hand_value):
    ...
    best_move = None
    best_draw_idx = -1
    current_hand_value = our_hand_value
